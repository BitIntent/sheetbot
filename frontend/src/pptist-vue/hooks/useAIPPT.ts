...
export default () => {
  ...
  const AIPPT = (templateSlides: Slide[], _AISlides: AIPPTSlide[], imgs?: ImgPoolItem[]) => {
    ...
    const fallbackTemplate = contentTemplates[0] || templateSlides[0] || null

    if (!transitionTemplate.value && transitionTemplates.length > 0) {
      transitionTemplate.value = transitionTemplates[0]
    }
    ...
    for (const item of AISlides) {
      if (item.type === 'cover') {
        const coverTemplate = coverTemplates.length
          ? coverTemplates[0]
          : fallbackTemplate
        if (!coverTemplate) continue
        ...
        slides.push({
          ...coverTemplate,
          id: nanoid(10),
          sheetbotSourceIndex: (item as any).source_index ?? -1,
          elements,
        })
      }
      else if (item.type === 'contents') {
        if (!contentsTemplates.length) continue
        const _contentsTemplates = getUseableTemplates(contentsTemplates, item.data.items.length, 'item')
        const contentsTemplate = _contentsTemplates[0]
        ...
        if (!endTemplates.length) continue
        const endTemplate = endTemplates[0]
